...
SpamThrottleSettings.gap = 120;		-- Default to allow message repeat once every 2 minutes (120 seconds)
...
--============================
local function SpamThrottle_ChannelMsgFilter(self, event, message, author, language, channelName, name2, flags, zoneId, channelIndex, channelBaseName, unused, lineId, ...)
	local BlockFlag = false;
	local Msg = "";
	
	if (author == nil or author == "") then 	-- if this is a server message (it has a playername)
		return
	end
	author = strsplit("-", author, 2) -- strips "-servername" from player name string
	...
	NormalizedMessage = SpamThrottle_strNorm(message,"")
	if SpamThrottle_SpamScoreBlock(message,NormalizedMessage,author) then
		BlockFlag = true;
	end
	...
	if (BlockFlag) then
		if (SpamThrottleSettings.filterMode == 1) then		-- Mode 1 means color the message gray, but don't hide it altogether
			...
			return false, ("|cFF5C5C5C" .. cleantext .. "|r"), author, language, channelName, name2, flags, zoneId, channelIndex, channelBaseName, unused, lineId, ...;
		else
			return true;	-- Mode 2, which is to hide the message entirely
		end
	end

	-- return false, ( "[" .. lineId .. "] " .. message), author, language, channelName, name2, flags, zoneId, channelIndex, channelBaseName, unused, lineId, ...;
	return false, message, author, language, channelName, name2, flags, zoneId, channelIndex, channelBaseName, unused, lineId, ...;
end
